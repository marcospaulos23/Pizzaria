...
interface SweetFlavorSelectorProps {
  selectedSize: string;
  sizeLabel: string;
  maxFlavors: number;
...
const SweetFlavorSelector = ({
  ...
  onBack,
  onSkip
}: SweetFlavorSelectorProps) => {
  const { pizzasDoces } = useMenu();
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <p className="text-sm text-muted-foreground">Pizza Doce - {sizeLabel}</p>
        </div>
      </div>
      ...
      {selectedFlavors.length > 0 && (
        <div className="flex flex-wrap gap-2 p-3 bg-secondary/10 rounded-xl border border-secondary/30">
      ...
      {/* Action buttons */}
      <div className="sticky bottom-0 pt-4 bg-card space-y-2">
        {selectedFlavors.length > 0 ? (
          <Button
            variant="cta"
            size="xl"
...
export default SweetFlavorSelector;
